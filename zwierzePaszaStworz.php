...
<div id="conteiner">
	<div class="Dane">
		<form action="zapiszaPaszaZwierzeSQL.php" method="post">
			Pasza: <br/>
 ...
 $pass="";
	...
	if($conn)
	{
		$querry="select * from dbo.Pasza";
		$result=sqlsrv_query($conn, $querry);
		if($result==true)
		{
			echo '<select name="pasza">';
 ...
 $pass="";
	...
	if($conn)
	{
		$querry="select dbo.Zwierze.IdZwierzencia, dbo.Zwierze.Gatunek , dbo.Zwierze.Rasa , dbo.Zwierze.Plec from Zwierze;";
		$result=sqlsrv_query($conn, $querry);
		if($result==true)
		{
			...
			{
				echo '<option>';
				echo $row['Gatunek']." ".$row['Rasa']." ".$row['Plec'];
				echo '</option>';
			}
			sqlsrv_free_stmt($result);
			sqlsrv_close($conn);
